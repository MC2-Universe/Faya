...
describe('UserAvatar', () => {
  describe('enable Auth', () => {
    ...
    it('should show Faya and default avatar when the user is not logged in and enable auth', () => {
      act(() => {
        useUserStore.setState({ enableAuth: () => true, isSignedIn: false, user: undefined });
      });

      render(<UserAvatar />);
      expect(screen.getByAltText('Faya')).toBeInTheDocument();
      expect(screen.getByAltText('Faya')).toHaveAttribute('src', DEFAULT_USER_AVATAR_URL);
    });
  });

  describe('disable Auth', () => {
    it('should show Faya and default avatar when the user is not logged in and disabled auth', () => {
      enableAuth = false;
      act(() => {
