...
const genProviderTable = (data: { desc?: string; id: string; name: string }) => {
  const title = genLink(data.name, urlJoin('https://faya.mc3.network/discover/provider', data.id));
  ...
  const newMd = updateReadme(
    ...
    [
      ...
      PROVIDER_LIST.slice(10, PROVIDER_LIST.length)
        ...
        .join('\n'),
      '</details>',
      `> 📊 Total providers: ${genLink(`<kbd>**${PROVIDER_LIST.length}**</kbd>`, 'https://faya.mc3.network/discover/providers')}`,
    ].join('\n\n'),
  );
